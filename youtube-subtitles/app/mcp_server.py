...
INSTRUCTIONS = (
    ...
    "always safe to call first: it needs no confirmation or user_confirmed flag. If your host requires the "
    "user to opt in to this connector before an app tool may run, do that in one short step and then call "
    "get_video_info right away, without asking what to do with the video. Its result says what to do next: either the interactive subtitle menu is shown "
