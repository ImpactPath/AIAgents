...
- **Summary report (.md)** asks Claude to fetch the subtitles and write its own structured summary as a downloadable Markdown file named after the video, plus a three-line summary in the chat. The prompt fixes the structure: a Video info section (title, channel, duration, published date, URL), a two-paragraph Overview, 8 to 10 numbered Key arguments with the speaker's reasoning, 6 to 8 paraphrased quotes, a Timeline of topics (approximate ranges when the subtitles carry no timestamps), and 5 to 7 thesis-first Takeaways, about 1,500 to 2,500 words for a 20 minute video. `docs/examples/summary-report-example.ko.md` is the report this format was modelled on.
- **Add to chat** fetches the selected track as TXT paragraphs and places it in the chat composer as a user message (`ui/message`) with a short instruction; the user presses send, and later questions use the transcript without another tool call. The instruction also tells the model to save the text unchanged as `<title>_subtitle_<lang>[_auto].txt` so it appears in the chat's files (unconditionally; hosts without file creation simply skip it). Claude Desktop accepts `ui/update-model-context` but the model did not see content sent that way, so the app does not use it.
- The prompt buttons (Summarize, Translate, Key points, Summary report) fill the chat composer with a prompt that spells out the expected structure (overview, ordered arguments with reasoning, and a takeaway for Summarize; a faithful complete translation in sections for Translate; 8 to 15 timestamped points under topic headings plus takeaways for Key points); the user sends the message and Claude then calls `get_subtitles`. The server answers that call from its caption cache (one hour by default), so YouTube is not contacted again for a track already previewed or downloaded. Every prompt first tells Claude to use the transcript if it is already in the conversation (after **Add to chat**, for example) and to fetch only if it is missing; this is stateless because the card is redrawn on each lookup.
