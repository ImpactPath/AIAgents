...
- **Summary report (.md)** asks Claude to fetch the subtitles and write its own structured summary (a Video section with title, channel, duration, published date and URL first, then overview, key arguments, paraphrased quotes, timeline, takeaways) as a downloadable Markdown file named after the video, plus a three-line summary in the chat.
