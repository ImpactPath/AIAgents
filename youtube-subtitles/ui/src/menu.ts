...
// Every prompt first points at a transcript already in the conversation (from Add to chat, a paste, or an
// earlier fetch) and fetches only when it is missing. This is stateless on purpose: a card is redrawn on
// every get_video_info call and on a page reload, so memory inside the card would be lost.
function promptText(kind: string): string {
  const t = currentTrack()!;
  const v = data!.video;
  const fetch =
    `If the subtitles of '${v.title}' (${t.lang}) are already in this conversation, use that text and do not ` +
