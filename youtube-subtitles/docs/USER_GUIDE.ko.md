...
| Translate to Korean(한국어로 번역) | 한국어 번역 요청문을 입력창에 채웁니다. 요약이나 생략 없이 전문을 번역하고, 길면 몇 문단씩 나누며, 이름과 전문 용어는 처음 나올 때 원어를 괄호로 병기하라고 지시합니다. | 서버 캐시 사용 |
| Key points(핵심 정리) | 핵심 정리 요청문을 채웁니다. 영상 순서대로 8~15개 요점을 한 문장씩, 자막에 시간이 있으면 [분:초]를 붙이고, 짧은 주제 제목으로 묶은 뒤 시사점 3개로 마무리하라고 지시합니다. | 서버 캐시 사용 |
| Summary report (.md)(요약 보고서) | 구조가 고정된 보고서를 Claude가 직접 써서 `.md` 파일로 내놓게 하는 요청문을 채웁니다. 순서는 영상 정보(제목, 채널, 길이, 게시일, URL), 두 문단 개요, 번호가 붙은 핵심 논지 8~10개(각각 발표자의 근거 포함), 인용 요지 6~8개(바꿔 쓴 문장), 주제 타임라인(자막에 시간이 없으면 대략적 구간), 시사점 5~7개(굵은 한 문장 명제와 설명)이며, 20분 영상 기준 1,500~2,500단어 분량입니다. 채팅에는 세 줄 요약이 함께 나옵니다. 기준이 된 예시는 `docs/examples/summary-report-example.ko.md`입니다. | 서버 캐시 사용 |
...
- 대화 제목은 Claude 앱이 첫 메시지를 보고 붙이므로 링크만 보내면 "YouTube video link" 같은 이름이 됩니다. 커넥터가 제목을 바꾸는 기능은 MCP 규격에 없으니, 첫 메시지에 주제어를 함께 쓰거나 대화 제목 메뉴에서 Rename으로 바꾸면 됩니다.
