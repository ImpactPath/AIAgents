...
| Summary report (.md)(요약 보고서) | 맨 앞의 영상 정보(제목, 채널, 길이, 게시일, URL)에 이어 개요, 핵심 논지, 인용 요지, 주제 타임라인, 시사점으로 구성된 보고서를 Claude가 직접 써서 `.md` 파일로 내놓게 하는 요청문을 채웁니다. 채팅에는 세 줄 요약이 함께 나옵니다. | 서버 캐시 사용 |
