...
자막 재조회에 대해서는 두 가지 장치가 있습니다. 서버는 조회한 영상 정보와 내려받은 자막 본문을 1시간 동안 기억하므로, 미리 보기 뒤에 요약을 눌러도 유튜브에는 다시 요청하지 않습니다. 또 Add to chat을 한 트랙은 이후 요약, 번역, 핵심 정리, 보고서 요청문이 "이미 대화에 있는 자막을 쓰라"고 지시하므로 도구 호출 자체가 생략됩니다. 한 영상으로 여러 질문을 할 계획이면 Add to chat을 먼저 누르는 것이 가장 경제적입니다.
